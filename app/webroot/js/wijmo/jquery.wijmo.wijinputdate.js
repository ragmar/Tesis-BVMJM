/*globals  wijDateTextProvider wijinputcore wijInputResult window document 
_dateDescriptor _dateDescriptor20 _dateDescriptor25 _dateDescriptor26 
_dateDescriptor27 alert _dateDescriptor1 _dateDescriptor10 _dateDescriptor100
_dateDescriptor101 _dateDescriptor2 _dateDescriptor250 _dateDescriptor30
_dateDescriptor251 _dateDescriptor31 _dateDescriptor45 _dateDescriptor46
_dateDescriptor47 _dateDescriptor48 _dateDescriptor50 _dateDescriptor51
_dateDescriptor60 _dateDescriptor61 Globalize jQuery*/
...
//		eight: 8
//	},
	var wijDateTextProvider, _iDateDescriptor, wijImplementInterface,
		_dateDescriptor, _dateDescriptor20, _dateDescriptor25, _dateDescriptor26,
		_dateDescriptor27, _dateDescriptor30, _dateDescriptor31,
		_dateDescriptor100, _dateDescriptor101, _dateDescriptor10,
		_dateDescriptor1, _dateDescriptor2, _dateDescriptor45, _dateDescriptor46,
		_dateDescriptor47, _dateDescriptor48, _dateDescriptor250,
		_dateDescriptor251, _dateDescriptor50, _dateDescriptor51,
		_dateDescriptor60, _dateDescriptor61;
	...
	$.widget("wijmo.wijinputdate", $.extend(true, {}, wijinputcore, {
		options: {
			...
			/// Type: Function.
			/// Code example: 
			/// $("#element").wijinputdate({ dateChanged: function (e, arg) { } });
			/// </summary>
			///
		...
		_highLightField: function (index) {
			if (index === undefined) {
				index = this.options.activeField;
			}
			if (this.isFocused()) {
			...
			case 191: // /
				if (e.shiftKey) {
					...
					if (this.options.activeField < 
						this._textProvider.getFieldCount() - 1) {
						this._toNextField();
						return true;
					}
				}
		...
		_keyPressPreview: function (e) {
			var key = e.keyCode || e.which, range, ch, fieldSep, cursor, now,
				newAction, lastTime, pos, ret;
			...
			if (range) {
				...
				fieldSep = this._textProvider
					.isFieldSep(ch, this.options.activeField);
				if (fieldSep) {
					this._toNextField();
					this._stopEvent(e);
					return true;
				...
				newAction = (now.getTime() - lastTime.getTime()) > 
				...
				ret = this._textProvider
					.addToField(ch, this.options.activeField, pos, !newAction);
				if (ret) {
					this._updateText();
		...
		_raiseDataChanged: function () {
			var d = this.options.date,
				prevDt = this.element.data('preDate');
			this.element.data('preDate', !d ? null : new Date(d.getTime()));

			if ((!prevDt && d) || (prevDt && !d) || 
				(prevDt && d && (prevDt.getTime() !== d.getTime()))) {
				this._syncCalendar();
		...
		_popupOrHideCalendar: function () {
			...
			calendar.wijcalendar('popup', $.extend({}, 
				this.options.popupPosition, { of: this.outerDiv }));
		},
	...
	wijDateTextProvider = function (w, f) {
		this.inputWidget = w;
	...
	wijDateTextProvider.prototype = {
		...
		needToMove: function (index, pos, ch) {
			...
			var desc = this.fields[index], val;
			if (pos === desc.maxLen) {
				return true;
			}
		...
		getDate: function () {
			return (!!this.inputWidget) ? 
				new Date(this.inputWidget._safeGetDate().getTime()) : undefined;
		},
		...
		_internalSetDate: function (date) {
			if (this.inputWidget) {
				var self = this,
					...
					typing = !!inputElement.data('typing'), chkBounds;
		...
		toString: function () {
			if (this.inputWidget.options.showNullText && 
				!this.inputWidget.isFocused() && this.inputWidget.isDateNull()) {
				return this.inputWidget.options.nullText;
			}

			var s = '', l = 0, i, txt, j, dp;
			this.desPostions = new Array(0);
		...
		addToField: function (input, activeField, pos, append) {
			...
			resultObj = { val: input, pos: 0, offset: 0, isreset: false };
		...
		insertAt: function (input, position, rh) {
			...
			rh.testPosition = -1;
			var desPos, oldTxt, pos, txt, tryToExpandAtRight, result,
				tryToExpandAtLeft, curInsertTxt, resultObj, prevTextLength,
				posAdjustValue, altInsertText, newTextLength, diff, s,
				delimOrEndPos, delta;
			if (input.length === 1) {
			...
			while (txt.length > 0 && pos < this.desPostions.length) {
				...
				if (desPos.desc.needAdjustInsertPos()) {
					...
					curInsertTxt = desPos.text.slice(0, desPos.pos) +
						curInsertTxt + desPos.text
						.slice(desPos.pos + curInsertTxt.length, desPos.length);
					...
					if (prevTextLength === 1 && curInsertTxt.length > 1 && 
						input.length === 1) {
						if (desPos.desc.type === 31 || desPos.desc.type === 25) {
							this._disableSmartInputMode = true;
						}
					}
					resultObj = { val: input, pos: desPos.pos, 
						offset: 0, isreset: false };
					result = desPos.desc.setText(curInsertTxt, 
						((input.length === 1) ? false : true), resultObj);
					this._disableSmartInputMode = false;
					if (!result && typeof (altInsertText) !== 'undefined' &&
						altInsertText.length > 0 && (desPos.desc.type === 26 ||
						desPos.desc.type === 27 || desPos.desc.type === 100 ||
						desPos.desc.type === 101 || desPos.desc.type === 250 || 
						desPos.desc.type === 251)) {
						result = desPos.desc.setText(altInsertText, 
							((input.length === 1) ? false : true), resultObj);
					}
					if (result) {
						rh.hint = rh.success;
						...
						if (input.length === 1) {
							...
							s = this.toString();
							rh.testPosition = desPos.desc.startIndex + 
								posAdjustValue + resultObj.offset;
						}
		...
		removeAt: function (start, end, rh) {
			try {
				...
				if (desPos.desc.needAdjustInsertPos()) {
					...
					curInsertTxt = desPos.text.slice(0, desPos.pos) + curInsertTxt + 
					desPos.text.slice(desPos.pos + curInsertTxt.length, desPos.length);
					resultObj = { val: curInsertTxt, pos: desPos.pos, offset: 0, 
		...
		_tryParseDate: function (val, pattern) {
			...
			sep = ci.standard["/"];
			patterns = [pattern, pattern2, pattern.replace(new RegExp(sep, 'g'), '-'),
		...
		_formatDate: function (d, f, ci) {
			...
			var self = this,
				cf = this.inputWidget._getCulture().calendars.standard,
			sRes = f.replace(new RegExp('yyyy|MMMM|MMM|MM|M|mm|m|dddd|ddd|dd|d|hh|h|HH|H|ss|s|tt|t|a/p', 'gi'), function (match) {
				var h;
				switch (match) {
				case 'yyyy':
				...
				case 't':
					return (d.getHours() < 12) ? ((cf.AM[0].length > 0) ?
						cf.AM[0].charAt(0) : '') : 
						((cf.PM[0].length > 0) ? cf.PM[0].charAt(0) : '');
				case 'a/p':
	...
	_iDateDescriptor = function (tp, id, type, len) {
	...
	wijImplementInterface = function (target, interfaceType) {
	...
	_dateDescriptor = function (owner, id) {
	...
	_dateDescriptor20 = function (owner, id) {
	...
	_dateDescriptor25 = function (owner, id) {
	...
	_dateDescriptor26 = function (owner, id) {
	...
	_dateDescriptor27 = function (owner, id) {
	...
	_dateDescriptor30 = function (owner, id) {
	...
	_dateDescriptor30.prototype = {
		...
		inc: function () {
			this._txtProvider
				.setDayOfMonth(this._txtProvider.getDayOfMonth() * 1 + 1, true);
		},
	...
	_dateDescriptor31 = function (owner, id) {
	...
	_dateDescriptor100 = function (owner, id) {
	...
	_dateDescriptor101 = function (owner, id) {
	...
	_dateDescriptor10 = function (owner, id) {
	...
	_dateDescriptor1 = function (owner, id) {
	...
	_dateDescriptor1.prototype = {
		...
		setText: function (value, allowchangeotherpart, result) {
			...
			var y = this._txtProvider.getYear(), m, dom, h, min, s;
			y = '' + y + '';
			if (value === '00') {
	...
	_dateDescriptor2 = function (owner, id) {
	...
	_dateDescriptor2.prototype = {
		...
		setText: function (value, allowchangeotherpart, result) {
			...
			var y = this._txtProvider.getYear(), m, dom, h, min, s, aRes;
			y = '' + y + '';
			if (value === '00') {
				m = this._txtProvider.getMonth();
				dom = this._txtProvider.getDayOfMonth();
			...
			aRes = this._txtProvider.setYear(y, result);
			return aRes;
		},
	...
	_dateDescriptor45 = function (owner, id) {
	...
	_dateDescriptor46 = function (owner, id) {
	...
	_dateDescriptor47 = function (owner, id) {
	...
	_dateDescriptor48 = function (owner, id) {
	...
	_dateDescriptor250 = function (owner, id) {
	...
	_dateDescriptor250.prototype = {
		...
		getText: function () {
			var h = this._txtProvider.getHours(), ds = '', 
				culture = this._txtProvider._getCulture();
			if (h < 12) {
	...
	_dateDescriptor251 = function (owner, id) {
	...
	_dateDescriptor50 = function (owner, id) {
	...
	_dateDescriptor51 = function (owner, id) {
	...
	_dateDescriptor60 = function (owner, id) {
	...
	_dateDescriptor61 = function (owner, id) {
