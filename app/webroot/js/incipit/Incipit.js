...
function CanvasClass ()
{
    ...
    this.getDrawPosition = function(context, elementX, elementY)
    {
        ...
        var positionY = (elementY + drawingProblemYPatchAmountFix + context.minStepY) * context.stepY - (context.stepY * contextMultiplier) + pixelsToAdd;
    ...
    {   
        ...
        for(var i=0; i < context.drawIncipitElements.length; i++)
        {
                ...
                if(context.drawIncipitElements[i].isClef)
                {
                    ...
                    for(var j=0; j < context.drawIncipitElements[i].qtyAlteration; j++)
                    {
                        ...
                        context.gDrawingContext.fillText(alteration.value, 
                                                        ...
                                                        + context.ratioY(context, positionAlteration.y - drawingProblemYPatchAmountFix));
                    }
                }
                else
                {
                    ...
                    context.gDrawingContext.fillText(alteration.value, 
                                                    notePosition.x, 
                                                    notePosition.y + context.ratioY(context, alteration.yPosition - drawingProblemYPatchAmountFix));
                }
            }
            ...
            if(context.drawIncipitElements[i].hasBar && !context.drawIncipitElements[i].isClef)
            {
                ...
                context.gDrawingContext.font = context.getFont(context, bar.font + drawingProblemYPatchAmountFix);
            ...
            if(context.drawIncipitElements[i].isClef)
            {
                ...
                if(drawingProblemYPatch)
                {
                    ...
                    notePosition = context.getDrawPosition(context, 
                                            context.drawIncipitElements[i].xPosition, 
                                            context.drawIncipitElements[i].yPosition + drawingProblemYPatchAmountFix);
                } 
            }else
            {
    ...
    this.TransformPAECToIncipit = function (context, paec)
    {
        ...
        for(var index = 0; index < paec.length; index++)
        {
            ...
            if(paec[index] == "@") //CIfra Indicadora de comp�s
            {
                var paecTime = "";
