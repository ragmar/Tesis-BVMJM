				...
				<td>
					<?php
						if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){
							if (($userItem['Item']['cover_name']) && (file_exists($_SERVER['DOCUMENT_ROOT'] . "/".$this->base."/webroot/attachments/files/small/" . $userItem['Item']['cover_path']))){
								echo $this->Html->image("/webroot/attachments/files/small/" . $userItem['Item']['cover_path'], array('title' => $userItem['Item']['cover_name']));
							} else {
								echo $this->Html->image("/webroot/img/sin_portada.jpg", array('width' => '5%'));
							}
						} else {
