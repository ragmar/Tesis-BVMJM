		...
		<form id="UserItemAddForm" name="UserItemAddForm" accept-charset="utf-8" method="post" action="<?php echo $this->base; ?>/user_items/add">
			...
			<?php if (!empty($item['Item']['item_file_path'])) { ?>
					<?php if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){ ?>
						<a href="http://<?php echo $_SERVER['HTTP_HOST'] . $this->base . '/webroot/attachments/files/' . $item['Item']['item_file_path']; ?>" class="btn-primary" target="_blank" title="Descargue el documento en su computadora.">Descargar e Imprimir Partitura</a>
					<?php } else { ?>
