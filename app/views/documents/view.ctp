		...
		<form id="UserItemAddForm" name="UserItemAddForm" accept-charset="utf-8" method="post" action="<?php echo $this->base; ?>/user_items/add">
			<?php
				if (($this->Session->check('Auth.User') && ($this->Session->read('Auth.User.group_id') != '3'))) {
					echo $this->Html->link('Agregar Documento', array('action' => '/add'), array('class' => 'btn-primary', 'title' => 'Agregar Documentos'));
					echo $this->Html->link('Modificar Documento', array('action' => '/edit/'.$item['Item']['id']), array('class' => 'btn-primary', 'title' => 'Modificar Documento'));
					echo $this->Html->link('Eliminar Documento', array('action' => '/delete/'.$item['Item']['id']), array('class' => 'btn-primary', 'title' => 'Eliminar Documento'));
				}
			?>
			<?php
			...
			<?php if (!empty($item['Item']['item_file_path']) && ($this->Session->check('Auth.User') && ($this->Session->read('Auth.User.group_id') != NULL))) { ?>
				<a href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . $this->base . '/app/webroot/files/' . $item['Item']['item_file_path']; ?>" class="btn-primary" target="_blank" title="Descargue el documento en su computadora.">Descargue Documento</a>
			<?php } ?>
		</form>
	</div>
