			...
			<?php
				if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){
					if (($item['Item']['cover_name']) && (file_exists($_SERVER['DOCUMENT_ROOT'] .$this->base."/app/webroot/covers/" . $item['Item']['cover_path']))){
						echo "<a href='".$this->base.'/academic_papers/view/'.$item['Item']['id']."'>".$this->Html->image("/app/webroot/covers/" . $item['Item']['cover_path'], array('width' => '90%', 'title' => $item['Item']['cover_name']))."</a>";
					} else {
						echo "<a href='".$this->base.'/academic_papers/view/'.$item['Item']['id']."'>".$this->Html->image("/app/webroot/img/sin_portada.jpg", array('width' => '90%'))."</a>";
					}
				} else {
