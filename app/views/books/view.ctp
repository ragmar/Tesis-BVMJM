...
<div class='book view'>
				...
				<?php
					if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){
						if (($item['Item']['cover_name']) && (file_exists($_SERVER['DOCUMENT_ROOT'] . "/".$this->base."/webroot/attachments/files/med/" . $item['Item']['cover_path']))){
							echo $this->Html->image("/webroot/attachments/files/med/" . $item['Item']['cover_path'], array('width' => '90%', 'title' => $item['Item']['cover_name']));
						} else {
			...
			<form id="UserItemAddForm" name="UserItemAddForm" accept-charset="utf-8" method="post" action="<?php echo $this->base; ?>/user_items/add">
				...
				<?php if (!empty($item['Item']['item_file_path'])) { ?>
					<?php if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){ ?>
						<a href="http://<?php echo $_SERVER['HTTP_HOST'] . $this->base . '/webroot/attachments/files/' . $item['Item']['item_file_path']; ?>" class="btn-primary" target="_blank" title="Descargue el documento en su computadora.">Descargar Documento</a>
					<?php } else { ?>
	...
	<div class="row">
		<table id="more" style="display: none; width: 100%;">
			...
			<tr>
				<td style="text-align: right; width: 50%;"><b><?php __('Mención de serie'); ?>:</b></td>
				<td style="padding-left: 1%; width: 50%;">
					<?php
						$serie = marc21_decode($item['Item']['490']);
						if (isset($serie['a'])) {echo $serie['a'] . ' ; '; $masdetalles++;}
						//if (isset($serie['l'])) {echo " " .$serie['l']; $masdetalles++;}
						if (isset($serie['v'])) {echo " " .$serie['v'] . ', '; $masdetalles++;}
						if (isset($serie['x'])) {echo $serie['x']; $masdetalles++;}
					?>
				</td>
			</tr>
			<?php } ?>
	...
	<?php if ($tipo[0] == "image") { ?>
		<?php if ($_SERVER['HTTP_HOST'] != "orpheus.human.ucv.ve"){ ?>
				<img src="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . $this->base . '/webroot/attachments/files/big/' . $item['Item']['item_file_path']; ?>" width="100%">
			<?php } else { ?>
