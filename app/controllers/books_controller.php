...
class BooksController extends AppController {
	...
	function index() {
		...
		if (!empty($this->data)) {
			...
			if (!empty($this->data['books']['Tipo'])) {
				$conditions['Item.h-007'] = $this->data['books']['Tipo'];
			}
		}
	...
	function add() {
		...
		if (!empty($this->data)) {
			...
			if ($_FILES['data']['error']['Book']['cover'] == 0){
				$this->Attachment->upload($this->data['Book'], 'cover');
			}

			if ($_FILES['data']['error']['Book']['item'] == 0){
				$this->Attachment->upload($this->data['Book'], 'item');
				
			}
			
			if ($_FILES['data']['error']['Book']['item'] == 0){
				$data['Book']['item_file_path'] = $this->data['Book']['item_file_path'];
				$data['Book']['item_content_type'] = $this->data['Book']['item_content_type'];
				$data['Book']['item_file_size'] = $this->data['Book']['item_file_size'];
				$data['Book']['item_file_name'] = $this->data['Book']['item_file_name'];
				$fileName = $data['Book']['item_file_path'];
			}

			if ($_FILES['data']['error']['Book']['cover'] == 0){
				$data['Book']['cover_path'] = $this->data['Book']['cover_file_path'];
				$data['Book']['cover_type'] = $this->data['Book']['cover_content_type'];
				$data['Book']['cover_size'] = $this->data['Book']['cover_file_size'];
				$data['Book']['cover_name'] = $this->data['Book']['cover_file_name'];
			}
			...
			if ($this->Item->save($data)) {
				...
				if ($_FILES['data']['error']['Book']['item'] == '1') {
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			...
			if ($_FILES['data']['error']['Book']['cover'] == 0){
				$item = $this->Item->find('first', array('conditions' => array('Item.id' => $data['Book']['id'])));
				$this->Attachment->delete_files($item['Item']['cover_path']); // Elimina el archivo anterior.
				$this->Attachment->upload($this->data['Book'], 'cover'); // Agrega el archivo nuevo.
			}
			
			if ($_FILES['data']['error']['Book']['item'] == 0){
				$item = $this->Item->find('first', array('conditions' => array('Item.id' => $data['Book']['id'])));
				$this->Attachment->delete_files($item['Item']['item_file_path']); // Elimina el archivo anterior.
				$this->Attachment->upload($this->data['Book'], 'item'); // Agrega el archivo nuevo.
			}
			...
			if ($this->Item->save($data)) {
				...
				if ($_FILES['data']['error']['Book']['item'] == '1') {
					$this->Session->setFlash(__('El archivo o documento no pudo ser modificado, verifique si sobrepasa los '.ini_get('upload_max_filesize').' permitidos.', true));
					$this->redirect(array('action' => 'edit/' . $data['Item']['id']));
				} else {
					$this->Session->setFlash(__('El libro ha sido modificado.', true));
					$this->redirect(array('action' => 'view/' . $data['Item']['id']));
				}
				
			} else {
				$this->Session->setFlash(__('El libro no pudo ser modificado. Por favor, intentélo nuevamente.', true));
				$this->redirect(array('action' => 'edit/' . $data['Item']['id']));
			}
