 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			return $this->Html->tag($tag, $this->link($title, $url, array_merge($options, compact('escape', 'class'))), array('class' => 'active'));
		}
	}
 ...
 */
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, $this->link($params['page'], array('page' => $params['page'])), array('class' => 'active'));
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i)), array('class' => 'active'));
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options));
				}
				if ($i != $params['pageCount']) {
